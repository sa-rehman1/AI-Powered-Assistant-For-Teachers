...
# Import helper functions from the new src/utils package
from src.utils.helpers import display_cards, query_chatgpt
...
# Streamlit app
def performance(): # Renamed the main function to 'performance' for clarity with the module name
    ...
    if uploaded_file is not None:
        if not OPENAI_KEY:
            st.error("Cannot perform analysis: OpenAI API Key is not configured.")
            return
        ...
        if not all(col in df.columns for col in required_columns):
            ...
            if analysis_type == "Student Wise Performance Analysis":
                ...
                selected_subject_to_improve = st.selectbox("Select a weak subject to get improvement suggestions:", weak_subjects)
                if selected_subject_to_improve:
                    st.write(f"**Suggestions to Improve Performance in {selected_subject_to_improve}:**")
                    subject_suggestions = get_subject_suggestions(selected_subject_to_improve)
                ...
                # The function query_chatgpt is now imported from src.utils.helpers
                ...
                if st.button("Get Answer"):
                    ...
                    # Query ChatGPT using the imported function
                    # We pass the OPENAI_KEY to the imported function
                    answer = query_chatgpt(question, context, OPENAI_KEY)
